...
def main():
    ...
    O conceito de RFV — Recência, Frequência e Valor — é utilizado para segmentar clientes com base em seus comportamentos de compra,
    agrupando-os em clusters com características semelhantes. Essa segmentação permite desenvolver ações de marketing e estratégias de CRM mais bem direcionadas,
    o que contribui tanto para a personalização do conteúdo quanto para a retenção de clientes.
    
    Para aplicar essa metodologia, é necessário calcular três componentes para cada cliente:

    - Recência (R):Número de dias desde a última compra.

    - Frequência (F): Total de compras realizadas dentro de um determinado período.

    - Valor (V): Soma do valor gasto nas compras durante esse mesmo intervalo.

    A seguir, vamos realizar esses cálculos.
    """)
    st.markdown("---")
    
    # Apresenta a imagem na barra lateral da aplicação
    st.sidebar.image('https://keystoneacademic-res.cloudinary.com/image/upload/element/97/97189_thumb.png')
